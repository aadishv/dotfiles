# Generated by Powerlevel10k configuration wizard on 2026-01-12 at 13:31 PST.
...
() {
  ...
  # CPU temperature prompt segment
  function prompt_cpu_temp() {
    emulate -L zsh
    local temp

    if command -v osx-cpu-temp >/dev/null 2>&1; then
      # Read raw first line of output (may contain degree symbol) and trim newline
      local full
      full=$(osx-cpu-temp -F 2>/dev/null | sed -n '1p' | tr -d '\n')
      # Prefer to use the full output so we keep the degree symbol (e.g. "94.1°F")
      temp="$full"
    else
      # osx-cpu-temp not installed; don't show the segment
      return
    fi

    if [[ -n $temp ]]; then
      # Remove any stray percent signs and ensure space before unit (e.g. "94.1 F")
      temp=${temp//%/}
      if [[ $temp == *F ]]; then
        temp="${temp%F} F"
      fi
      local fg=${POWERLEVEL9K_CPU_TEMP_FOREGROUND:-7}
      local bg=${POWERLEVEL9K_CPU_TEMP_BACKGROUND:-4}
      p10k segment -b $bg -f $fg -t "$temp"
    fi
  }
  ...
  typeset -g POWERLEVEL9K_CPU_TEMP_FOREGROUND=7
  typeset -g POWERLEVEL9K_CPU_TEMP_BACKGROUND=4
