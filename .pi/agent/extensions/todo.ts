...
function resolveCwd(cwd: string | undefined): string {
	// In some non-interactive contexts, ctx.cwd may be missing.
	// Fall back to process.cwd() to avoid path.resolve(undefined, ...).
	return cwd && cwd.trim().length > 0 ? cwd : process.cwd();
}

function getTodosDir(cwd: string | undefined): string {
	const baseCwd = resolveCwd(cwd);
	const overridePath = process.env[TODO_PATH_ENV];
	if (overridePath && overridePath.trim()) {
		return path.resolve(baseCwd, overridePath.trim());
	}
	return path.resolve(baseCwd, TODO_DIR_NAME);
}

function getTodosDirLabel(cwd: string | undefined): string {
	const baseCwd = resolveCwd(cwd);
	const overridePath = process.env[TODO_PATH_ENV];
	if (overridePath && overridePath.trim()) {
		return path.resolve(baseCwd, overridePath.trim());
